...
public class TuringMachineUI {
	...
	public void enterTape() {
		...
		if (isValid) {
			...
			System.out.println("You entered an unvalid set of values.");
	}

	public void enterCommand() {
		String message = "Enter a command 'ABCDE' where:\n";
		message += "A is one of the following values: '0', '1', '_'.\n";
		message += "B is a letter from A to Z.\n";
		message += "C is one of the following values: '0', '1', '_', '*'.\n";
		...
		boolean isEqualToStatus = Character.toUpperCase(commandArray[1]) == Character.toUpperCase(turingMachine.getStatus());
		boolean isEntryValid = (commandArray[2] == '0') || (commandArray[2] == '1') || (commandArray[2] == '_') || (commandArray[2] == '*');
		boolean isDirectionValid = (Character.toUpperCase(commandArray[3]) == 'L') || (Character.toUpperCase(commandArray[3]) == 'R');
		...
		if (isEqualToPointer && isEqualToStatus && isEntryValid && isDirectionValid && isFinalStatusValid)
			...
			showAlert(isEqualToPointer, isEqualToStatus, isEntryValid, isDirectionValid, isFinalStatusValid);
	}

	public void executeCommand(char[] command) {
		if (command[2] != '*')
			turingMachine.setTapeValue(command[2]);

		movePointer(command[3]);

		turingMachine.setStatus(Character.toUpperCase(command[4]));
	}
	...
	public void movePointer(char movement) {
		...
		switch (movement) {
			case 'L':
				if (pointer <= 0)
					turingMachine.setPointer((tape.length - 1) + pointer);
				else
					turingMachine.setPointer(pointer - 1);
				break;

			case 'R':
				if (pointer >= tape.length - 1)
					turingMachine.setPointer(pointer - (tape.length - 1));
				else
					turingMachine.setPointer(pointer + 1);
				break;
	...
	public void showAlert(boolean isEqualToPointer, boolean isEqualToStatus, boolean isEntryValid,
			boolean isDirectionValid, boolean isFinalStatusValid) {
		String message = "The command could not be executed.\n";
		if (!isEqualToPointer)
			message += "The first parameter has to be equal to the pointer value.\n";
		if (!isEqualToStatus)
			message += "The second parameter has to be equal to the machine's status.\n";
		if (!isEntryValid)
			message += "The third parameter wasn't a valid value (You have to enter one of the following values: '0', '1', '_', '*').\n";
		if (!isDirectionValid)
			message += "The fourth parameter wasn't a valid value (You have to enter either 'L' or 'R').\n";
		if (!isFinalStatusValid)
			message += "The fifth parameter wasn't a valid value (You have to enter a letter from A to Z).\n";
